...
export async function createEntrega(formData: FormData) {
  ...
  const scheduledPeriod =
    (formData.get("scheduled_period") as string) || null;
  ...
  const interestedName =
    (formData.get("interested_name") as string) || null;
  const interestedNote =
    (formData.get("interested_note") as string) || null;
...
export async function assignEntregador(
  entregaId: string,
  entregadorId: string
) {
  const supabase = await createClient();
  ...
  await supabase
    ...
    .update({
      ...
      status: "rota_definida" as DeliveryStatus,
    })
    .eq("id", entregaId);
